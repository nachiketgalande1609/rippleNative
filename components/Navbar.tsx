...
function useToastStack() {
  ...
  const timerMap = useRef<Map<number, ReturnType<typeof setTimeout>>>(
    new Map(),
  );
    ...
    (preview: MessagePreview) => {
      setToasts((prev) => {
        const existingIdx = prev.findIndex(
          (t) => t.preview.senderId === preview.senderId,
        );
        if (existingIdx !== -1) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <Text
            style={[styles.toastUsername, { color: colors.textPrimary }]}
            numberOfLines={1}
          >
...
export default function NavBar({
  ...
  const NavBtn = ({ item }: { item: NavItemDef }) => {
    ...
    return (
      <TouchableOpacity
        onPress={() =>
          router.push(item.segment === "" ? "/" : `/${item.segment}`)
        }
        style={styles.navBtn}
        activeOpacity={0.7}
      >
  ...
  return (
    <>
      ...
      >
        <NavBtn
          item={{ segment: "", icon: "home-outline", activeIcon: "home" }}
        />
        <NavBtn
          item={{
            segment: "search",
