        ...
        (preview: MessagePreview) => {
            setToasts((prev) => {
                ...
                if (existingIdx !== -1) {
                    ...
                    const next = [...prev];
                    next[existingIdx] = { ...existing, preview: { ...preview }, hiding: false, version: existing.version + 1 };
                    return next;
                }
...
function ToastBanner({
    ...
    return (
        <Animated.View
            style={[
                styles.toastBanner,
                { backgroundColor: colors.surface, borderColor: colors.border, top: insetTop + 8, opacity, transform: [{ translateY }] },
            ]}
        >
            <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => { onDismiss(toast.id); onPress(toast.preview.senderId); }}
                style={styles.toastInner}
            >
                <View style={{ position: "relative" }}>
                    <Image
                        source={
                            toast.preview.senderProfilePicture
                                ? { uri: toast.preview.senderProfilePicture }
            ...
            {/* Progress bar */}
            <Animated.View
                style={[styles.toastProgress, { width: progress.interpolate({ inputRange: [0, 1], outputRange: ["0%", "100%"] }) }]}
            />
        </Animated.View>
    );
}
...
export default function NavBar({
    ...
    useEffect(() => {
        ...
        return () => { socket.off("unreadMessagesCount", handler); };
    }, [push, pathname]);
    ...
    const isActive = (segment: string) =>
        segment === "messages"
            ...
            : pathname === `/${segment}` || (segment === "" && pathname === "/");

    if (hideBar) return null;

    // ── Nav items ──
    type NavItemDef = { segment: string; icon: string; activeIcon: string; badge?: number | null };
    ...
    const loggedInItems: NavItemDef[] = [
        ...
        { segment: "messages", icon: "chatbubble-outline", activeIcon: "chatbubble", badge: unreadMessagesCount },
        { segment: "notifications", icon: "heart-outline", activeIcon: "heart", badge: unreadNotificationsCount },
    ];

    const loggedOutItems: NavItemDef[] = [
        { segment: "login", icon: "log-in-outline", activeIcon: "log-in" },
        { segment: "register", icon: "person-add-outline", activeIcon: "person-add" },
    ];
    ...
    const NavBtn = ({ item }: { item: NavItemDef }) => {
        ...
        return (
            <TouchableOpacity
                onPress={() => router.push(item.segment === "" ? "/" : `/${item.segment}`)}
                style={styles.navBtn}
                activeOpacity={0.7}
            >
                <View>
        ...
        <>
            ...
            >
                ...
                <View style={styles.navSide}>
                    {leftItems.map((item) => (
                        <NavBtn key={item.segment} item={item} />
                    ))}
                </View>

                {/* Centre: create post (logged in) or empty */}
                {currentUser ? (
                    <TouchableOpacity
                ...
                <View style={styles.navSide}>
                        ...
                        >
                            <Image
                                ...
                                style={[
                                    ...
                                    isActive(`profile/${currentUser.id}`) && styles.profileAvatarActive,
                                ]}
                            />
                        </TouchableOpacity>
                    )}
                    ...
                    {currentUser && (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.7}
                        >
                            <Ionicons name="log-out-outline" size={22} color={colors.textDisabled} />
                        </TouchableOpacity>
                    )}
                </View>
            </View>
...
const styles = StyleSheet.create({
    // Nav bar
    navBar: { position: "absolute", bottom: 0, left: 0, right: 0, flexDirection: "row", alignItems: "center", borderTopWidth: 1, paddingTop: 8, paddingHorizontal: 4, zIndex: 100 },
    navSide: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "space-evenly" },
    navBtn: { padding: 8, alignItems: "center", justifyContent: "center" },

    // Badge
    badge: { position: "absolute", top: -4, right: -6, backgroundColor: "#e53935", borderRadius: 8, minWidth: 15, height: 15, alignItems: "center", justifyContent: "center", paddingHorizontal: 2 },
    badgeText: { color: "#fff", fontSize: 9, fontWeight: "600" },

    // Create btn
    createBtn: { width: 44, height: 44, borderRadius: 13, backgroundColor: ACCENT, alignItems: "center", justifyContent: "center", marginBottom: 4 },

    // Profile avatar
    profileAvatar: { width: 26, height: 26, borderRadius: 13, borderWidth: 1.5, borderColor: "transparent" },
    profileAvatarActive: { borderColor: ACCENT },

    // Toast banner
    toastBanner: { position: "absolute", left: 10, right: 10, zIndex: 1500, borderRadius: 14, borderWidth: 1, overflow: "hidden", shadowColor: "#000", shadowOpacity: 0.12, shadowRadius: 12, shadowOffset: { width: 0, height: 4 }, elevation: 8 },
    toastInner: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 14, paddingVertical: 12 },
    toastAvatar: { width: 38, height: 38, borderRadius: 19 },
    toastOnlineDot: { position: "absolute", bottom: 0, right: 0, width: 10, height: 10, borderRadius: 5, backgroundColor: ACCENT, borderWidth: 2, borderColor: "#fff" },
    toastUsername: { fontWeight: "600", fontSize: 13, lineHeight: 18 },
    toastMessage: { fontSize: 12.5, lineHeight: 18 },
    toastClose: { padding: 4 },
    toastProgress: { height: 2, backgroundColor: ACCENT, position: "absolute", bottom: 0, left: 0 },
    ...
    logoutBtn: { flex: 1, borderWidth: 1, borderRadius: 10, paddingVertical: 10, alignItems: "center" },
