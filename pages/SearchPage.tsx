...
function TagHistoryRow({ item, onPress, onDelete, colors }: { item: any; onPress: () => void; onDelete: () => void; colors: any }) {
    return (
        <TouchableOpacity onPress={onPress} activeOpacity={0.7} style={styles.userRow}>
            ...
            <Text style={[styles.userName, { color: colors.textPrimary }]}>#{item.tag}</Text>
            <TouchableOpacity
                onPress={onDelete}
                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
                style={[styles.deleteBtn, { backgroundColor: colors.hover }]}
...
export default function SearchPage() {
    ...
    useEffect(() => {
        const run = async () => {
            if (!debouncedUserQuery) { setUserResults([]); return; }
            setUserLoading(true);
            try {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            if (!raw.trim()) { setTagResults([]); return; }
            setTagLoading(true);
            try {
    ...
    const handleDeleteUserHistory = async (historyId: number) => {
        ...
        try { await deleteSearchHistoryItem(historyId); }
        catch { if (deleted) setHistory((p) => [deleted, ...p]); }
    };
    ...
    const handleDeleteTagHistory = async (historyId: number) => {
        ...
        try { await deleteHashtagSearchHistoryItem(historyId); }
        catch { if (deleted) setTagHistory((p) => [deleted, ...p]); }
    };
    ...
    const PeopleContent = () => {
        if (userLoading && isUserSearching) {
            return <>{[1, 2, 3].map((i) => <SkeletonRow key={i} colors={colors} />)}</>;
        }
        ...
        if (history.length > 0) {
            return (
                <>
                    <SectionHeader
                        icon={<Ionicons name="time-outline" size={15} color={colors.textDisabled} />}
                        label="Recent"
                        colors={colors}
                    />
                    {history.map((item) => (
        ...
        <SafeAreaView style={[styles.root, { backgroundColor: colors.bg }]} edges={["top"]}>
            ...
            <View style={[styles.header, { backgroundColor: colors.bg, borderBottomColor: colors.border }]}>
                ...
                <View style={styles.inputWrapper}>
                    <View style={[
                        styles.inputRow,
                        { backgroundColor: colors.surface, borderColor: activeTab === 1 ? ACCENT + "40" : colors.border },
                    ]}>
                        {activeTab === 0 ? (
                ...
                <View style={[styles.tabs, { borderBottomColor: colors.border }]}>
                    {[
                        ...
                        <TouchableOpacity
                            ...
                            style={[
                                styles.tab,
                                activeTab === i && { borderBottomWidth: 1.5, borderBottomColor: ACCENT },
                            ]}
                        >
                            <Ionicons name={tab.icon} size={14} color={activeTab === i ? colors.textPrimary : colors.textDisabled} />
                            <Text style={[styles.tabLabel, { color: activeTab === i ? colors.textPrimary : colors.textDisabled, fontWeight: activeTab === i ? "600" : "500" }]}>
                                {tab.label}
                            </Text>
                        </TouchableOpacity>
                    ))}
            ...
            {/* ── Content ── */}
            <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                <View style={{ paddingTop: 8 }}>
                    {activeTab === 0 ? <PeopleContent /> : <HashtagContent />}
                </View>
            </ScrollView>
        </SafeAreaView>
    );
...
const styles = StyleSheet.create({
    ...
    inputWrapper: { paddingHorizontal: 12, paddingTop: 12, paddingBottom: 10, position: "relative" },
    inputRow: { flexDirection: "row", alignItems: "center", gap: 8, borderRadius: 10, borderWidth: 1, paddingHorizontal: 12, height: 42 },
    input: { flex: 1, fontSize: 14.5 },
    loadingBar: { position: "absolute", bottom: 0, left: 12, right: 12, height: 2, overflow: "hidden", borderRadius: 1 },
    loadingBarFill: { height: "100%", width: "60%", opacity: 0.7 },

    // Tabs
    tabs: { flexDirection: "row", borderBottomWidth: 1 },
    tab: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, height: 40, borderBottomWidth: 0, borderBottomColor: "transparent" },
    tabLabel: { fontSize: 13.5, letterSpacing: 0.1 },

    // Section header
    sectionHeader: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4 },
    sectionLabel: { fontSize: 11, fontWeight: "600", letterSpacing: 0.9 },

    // Skeleton
    skeletonRow: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 16, paddingVertical: 10 },
    ...
    // Tag
    tagIcon: { width: 38, height: 38, borderRadius: 19, borderWidth: 1, alignItems: "center", justifyContent: "center" },
    ...
    // Grid
    postCount: { fontSize: 11, fontWeight: "600", letterSpacing: 0.9, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 8 },
    grid: { flexDirection: "row", flexWrap: "wrap", gap: 2 },
    gridCell: { width: CELL, height: CELL, overflow: "hidden" },
    gridImage: { width: "100%", height: "100%" },
    videoIndicator: { position: "absolute", top: 5, right: 5, backgroundColor: "rgba(0,0,0,0.5)", borderRadius: 8, padding: 3 },
});