...
const MessagesPage: React.FC<MessagesPageProps> = ({ handleVideoCall = () => {} }) => {
    ...
    const { onlineUsers, onVideoCall } = useGlobalStore();
    ...
    useEffect(() => {
        ...
        socket.on("typing", (data) => {
            if (data.receiverId === currentUser.id && selectedUser?.id === data.senderId) setTypingUser(data.senderId);
        });
    ...
    useEffect(() => {
        ...
        const hasUnread = messages.some((m) => m.sender_id === selectedUser.id && !m.read);
        if (hasUnread) {
    ...
    return (
        ...
        >
            {!selectedUser ? (
                ...
                <KeyboardAvoidingView style={styles.chatView} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={0}>
                    <MessagesTopBar
                        selectedUser={selectedUser}
                        openVideoCall={() => selectedUser && onVideoCall?.(selectedUser.id)}
                        onMuteToggle={() => {}}
                        onBack={() => {
