        ...
        <Animated.View style={[styles.skeletonFill, { opacity: pulse }]}>
            {[true, false, true, false, true, false, true].map((self, i) => (
                <View key={i} style={[styles.skeletonRow, { justifyContent: self ? "flex-end" : "flex-start" }]}>
                    ...
                    <View style={[styles.skeletonBubble, { backgroundColor: colors.hover, width: 80 + i * 22 }]} />
                </View>
            ))}
        </Animated.View>
    );
...
function MessageDetailsSheet({
    ...
    const readTs = msg.read ? new Date(new Date(msg.timestamp).getTime() + 15000).toISOString() : null;
            ...
            <Pressable style={styles.sheetBackdrop} onPress={onClose}>
                ...
                >
                    <Pressable onPress={(e) => e.stopPropagation()}>
                        ...
                        {/* Title */}
                        <Text style={[styles.sheetTitle, { color: colors.textPrimary }]}>Message info</Text>
                        ...
                        {rows.map((row, i) => (
                            <View
                                key={i}
                                style={[styles.detailRow, { borderBottomColor: colors.border }, i === rows.length - 1 && { borderBottomWidth: 0 }]}
                            >
                                <View style={[styles.detailIconWrap, { backgroundColor: row.bgColor }]}>
                                    <Ionicons name={row.icon as any} size={15} color={row.pending ? colors.textDisabled : row.iconColor} />
                                </View>
                                <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>{row.label}</Text>
                                {row.pending || !row.info ? (
                                    <Text style={[styles.detailPending, { color: colors.textDisabled }]}>Pending</Text>
                                ) : (
                                    <View style={styles.detailTimeWrap}>
                                        <Text style={[styles.detailTime, { color: colors.textPrimary }]}>{row.info.time}</Text>
                                        <Text style={[styles.detailDate, { color: colors.textDisabled }]}>{row.info.date}</Text>
                                    </View>
                                )}
                            </View>
                        ))}
...
function PopupMenu({
    ...
    const popupTop = showAbove ? position.top - EMOJI_H - MENU_H - GAP * 2 : position.top + position.height + GAP;
                ...
                >
                    ...
                    >
                        {REACTIONS.map((emoji, i) => (
                            <TouchableOpacity key={i} onPress={() => onReact(emoji)} style={styles.emojiBtnWrap} activeOpacity={0.7}>
                    ...
                    >
                        {menuItems.map((item, i) => (
                            ...
                            >
                                <Ionicons name={item.icon} size={16} color={(item as any).danger ? "#e53935" : colors.textSecondary} />
                                <Text
                                    style={[
                                        styles.actionLabel,
                                        {
...
function MessageBubble({
    ...
    return (
        <View style={{ marginBottom: 10 }}>
            ...
            <View style={[styles.msgRow, { justifyContent: self ? "flex-end" : "flex-start" }]}>
                ...
                >
                    ...
                    {originalMessage && (
                        ...
                        >
                            <Text style={styles.replyQuoteUser}>{originalMessage.sender_id === currentUser.id ? "You" : selectedUser.username}</Text>
                                    ...
                                    style={{
                                        ...
                                        height:
                                            msg.post.media_width && msg.post.media_height
                                                ? (msg.post.media_height / msg.post.media_width) * mediaW
                                                : mediaW,
                                    }}
                                    resizeMode="cover"
                                />
                    ...
                    {/* Text bubble */}
                    {!!msg.message_text && (
                        <TouchableOpacity ref={bubbleRef as any} onLongPress={openPopup} activeOpacity={1} delayLongPress={280}>
                            <View
                                style={[
                                    styles.bubble,
                                    {
                            ...
                            >
                                <Text style={[styles.bubbleText, { color: self ? "#fff" : colors.textPrimary }]}>{msg.message_text}</Text>
                                <Text
                                    style={[
                                        styles.bubbleTime,
                                        {
            ...
            {/* ── Message Details Sheet ── */}
            <MessageDetailsSheet visible={showDetails} msg={msg} colors={colors} isDark={isDark} onClose={() => setShowDetails(false)} />
        </View>
    );
}
...
const styles = StyleSheet.create({
    ...
    typingDot: { width: 4, height: 4, borderRadius: 3 },
