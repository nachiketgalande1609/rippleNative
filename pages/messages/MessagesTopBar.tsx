    ...
    return (
        <>
            <View style={[styles.bar, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
                ...
                <TouchableOpacity onPress={() => router.push(`/profile/${selectedUser?.id}`)} style={styles.userInfo} activeOpacity={0.8}>
                    <View style={styles.avatarWrap}>
                        ...
                        />
                        {/* Online dot */}
                        {isOnline && <View style={[styles.onlineDot, { borderColor: colors.surface }]} />}
                    </View>
                    <View style={{ minWidth: 0, justifyContent: "center" }}>
                        <Text style={[styles.username, { color: colors.textPrimary }]} numberOfLines={1}>
                            {selectedUser?.username}
                        </Text>
                        <Text style={[styles.onlineText, { color: colors.textDisabled }]}>{isOnline ? "Online" : "Offline"}</Text>
                    </View>
                </TouchableOpacity>

                {/* Right actions */}
                <View style={styles.actions}>
                    <TouchableOpacity onPress={openVideoCall} style={[styles.actionBtn, { backgroundColor: colors.hover }]} activeOpacity={0.7}>
                ...
                <Pressable style={styles.backdrop} onPress={() => setOptionsOpen(false)}>
                    <Pressable>
                        <View style={[styles.optionsCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                            {/* Mute / Unmute */}
                            <TouchableOpacity onPress={handleToggleMute} activeOpacity={0.7} style={styles.optionRow}>
                                <View
                                    style={[
                                        styles.optionIcon,
                                        {
                                            backgroundColor: isMuted ? "rgba(124,92,252,0.12)" : colors.hover,
                                        },
                                    ]}
                                >
                                    <Ionicons
                                ...
                                <View style={{ flex: 1 }}>
                                    <Text style={[styles.optionLabel, { color: colors.textPrimary }]}>
                                        {muteLoading ? "Updating…" : isMuted ? "Unmute notifications" : "Mute notifications"}
                                    </Text>
                                    <Text style={[styles.optionSub, { color: colors.textDisabled }]}>
                                        {isMuted ? "Turn notifications back on" : `Silence messages from ${selectedUser?.username}`}
                                    </Text>
                                </View>
                                {isMuted && (
                            ...
                            {/* Cancel */}
                            <TouchableOpacity onPress={() => setOptionsOpen(false)} activeOpacity={0.7} style={styles.optionRow}>
                                <View style={[styles.optionIcon, { backgroundColor: colors.hover }]}>
                                    <Ionicons name="close" size={18} color={colors.textDisabled} />
                                </View>
                                <Text style={[styles.optionLabel, { color: colors.textDisabled }]}>Cancel</Text>
                            </TouchableOpacity>
                        </View>
                    </Pressable>
                </Pressable>
